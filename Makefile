...
# platform:  $XFree86: xc/config/cf/linux.cf,v 3.200 2003/02/16 23:59:07 alanh Exp $

# operating system:  Linux 2.4.20-13.9 i686 [ELF] (2.4.20)
# libc:	(6.3.2)
# binutils:	(213)
...
# $XFree86: xc/config/cf/xfree86.cf,v 3.438 2003/02/26 09:21:32 dawes Exp $
...
.SUFFIXES: .cc
          ...
          CXXFILT = c++filt
           CXXLIB =
    CXXDEBUGFLAGS = -O2  -pipe -march=i386 -mcpu=i686 -fno-strength-reduce -fno-strict-aliasing -pipe
CXXDEPENDINCLUDES =
 CXXEXTRA_DEFINES =
CXXEXTRA_INCLUDES =
...
#
#
# $XFree86: xc/config/cf/X11.tmpl,v 1.195 2003/02/17 18:57:46 dawes Exp $
...
# Personal.tmpl
#
#	$Id: Makefile 610 2005-03-15 19:42:08Z hussain $ (John Heidemann) $Date: 2005-03-15 11:42:08 -0800 (Tue, 15 Mar 2005) $
#
...
# BEGIN User customizable things
                ...
                XCONFIG = /var/X11R6/lib/config/Imake.tmpl
          ...
          IMAKE_DEFINES = -DDefaultCDebugFlags=-g  -I$(HOMELIB) -I/usr/local/lib/X11/config

                  LATEX = pdflatex

               HTMLARGS = -split 0 -info "" -html_version 3.2 -no_navigation

# DVIPS_OPTS should make output go to stdout.
# SCREEN_OPTS "-P FicusVGA" removed 15-Jul-97.

     DVIPS_PRINTER_OPTS = -t Letter -f -Ppdf -G0
      DVIPS_SCREEN_OPTS = -f
        DVIPS_FILE_OPTS = -Z

           PRESENT_ARGS = -noantialias

TEXFILES = paper.tex abstract.tex intro.tex relatedwork.tex data.tex function.tex character.tex terms.tex conclusion.tex 
...
paper.ps.gz: paper.dvi $(TEXFILES)
	dvips $(DVIPS_PRINTER_OPTS) $(DVIPS_FILE_OPTS) paper |gzip >paper.ps.gz

paper.ps: paper.dvi $(TEXFILES)
	dvips $(DVIPS_PRINTER_OPTS) $(DVIPS_FILE_OPTS) paper >paper.ps

paper.pdf: paper.ps paper.dvi $(TEXFILES)
	ps2pdf paper.ps paper.pdf

paper.text: paper.dvi $(TEXFILES)
	dvidoc paper.dvi >paper.text

paper/paper.html:: paper.aux $(TEXFILES)
	latex2html $(HTMLARGS) paper.tex

bibtex.paper:: paper.aux $(TEXFILES)
	bibtex paper

rebibtex.paper:: paper.aux $(TEXFILES)
	make latex.paper
	make bibtex.paper
	make latex.paper
	make latex.paper

xdvi.paper: paper.dvi $(TEXFILES)
	xdvi paper

ghostview.paper:: paper.dvi $(TEXFILES)
	dvips $(DVIPS_PRINTER_OPTS) paper|tmpfile ghostview

gv.paper:: paper.dvi $(TEXFILES)
	dvips $(DVIPS_PRINTER_OPTS) paper|tmpfile gv

xpdf.paper:: paper.pdf paper.ps paper.dvi $(TEXFILES)
	xpdf paper.pdf

paper.present.ps.gz:: paper.dvi $(TEXFILES)
	dvips $(DVIPS_SCREEN_OPTS) paper|gzip >paper.present.ps.gz

oldpresent.paper:: paper.dvi $(TEXFILES)
	zcat paper.present.ps.gz|tmpfile ghostview -name presentation

present640.paper:: paper.dvi $(TEXFILES)
	zcat paper.present.ps.gz|tmpfile gv -geometry 767x551+535+-75 $(PRESENT_ARGS)

present800.paper:: paper.dvi $(TEXFILES)
	zcat paper.present.ps.gz|tmpfile gv -geometry 1093x823+823-76 $(PRESENT_ARGS) -magstep 1

present1024.paper:: paper.dvi $(TEXFILES)
	zcat paper.present.ps.gz|tmpfile gv -geometry 1167x902+911--57 $(PRESENT_ARGS) -magstep 2

print.paper:: paper.dvi $(TEXFILES)
	dvips $(DVIPS_PRINTER_OPTS) paper | lpr

cleandir::
	$(RM) paper.{blg,log}

wipe:: cleandir
	$(RM) paper.{aux,bbl,dvi,ps}

latex.paper::
	$(LATEX) paper
	@if egrep 'Rerun to get' paper.log;	then	echo 'Relatexing for cross-references.';	$(LATEX) paper;	fi
...
rebibtex:: rebibtex.paper

xdvi:: xdvi.paper

ghostview:: ghostview.paper

gv:: gv.paper

xpdf:: xpdf.paper

present:: present800.paper

present640:: present640.paper

present800:: present800.paper

present1024:: present1024.paper
